...
// UI Components
import { 
  X, ArrowRight, AlertCircle, Lock, Mail, User, 
  Eye, EyeOff, Loader2, Apple, CheckCircle2, Smartphone, MessageSquare, ArrowLeft
} from "lucide-react";
import loginImg from "../../assets/common/logind.webp";
    ...
    <div className="fixed inset-0 z-[10000] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full sm:max-w-[340px] p-6 rounded-t-3xl sm:rounded-2xl shadow-2xl relative animate-in slide-in-from-bottom-10 duration-300">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-1">
          {step === 'PHONE' ? 'Phone Register' : 'Verify Code'}
        </h3>
        <p className="text-slate-500 text-xs mb-6">
          {step === 'PHONE' ? 'Create account via SMS.' : `Code sent to ${phoneNumber}`}
        </p>
...
const Register = () => {
  ...
  const handleGoogleLogin = async () => {
    try { await googleSignUp(); navigate("/"); } catch (e) { console.error(e); }
  };

  const handleAppleLogin = async () => {
    try { await appleSignUp(); navigate("/"); } catch (e) { console.error(e); }
  };
    ...
    try {
      ...
      try {
        ...
        if (createErr.code === 'auth/email-already-in-use') {
          try {
            const loginCredential = await signInWithEmailAndPassword(secondaryAuth, data.email, data.password);
            user = loginCredential.user;
            if (user.emailVerified) throw createErr; 
          } catch (loginErr) { throw createErr; }
        } else {
          throw createErr;
        }
      }
  ...
  return (
    ...
    <div className="fixed inset-0 w-screen h-[100dvh] flex bg-white font-sans overflow-hidden">
      ...
      {/* -----------------------------
          LEFT SIDE: FORM (Full Width Mobile)
         ----------------------------- */}
              ...
              <>
                ...
                <div className="space-y-3 mb-6">
                  <button type="button" onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-full border border-slate-300 bg-white hover:bg-slate-50 hover:border-slate-400 transition-all group shadow-sm">
                    ...
                    <span className="text-slate-600 font-medium text-sm group-hover:text-slate-900">Sign up with Google</span>
                  </button>
                ...
                <div className="relative flex py-2 items-center mb-6">
                  ...
                  <span className="flex-shrink-0 mx-4 text-slate-400 text-[10px] font-bold uppercase tracking-widest">or register with email</span>
                  <div className="flex-grow border-t border-slate-200"></div>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <div className="relative group">
                    <input
                      id="name"
                      type="text"
                      className={`peer block w-full px-4 py-3 bg-white border border-slate-300 rounded-lg text-sm text-slate-900 placeholder-transparent focus:border-[#0f766e] focus:ring-1 focus:ring-[#0f766e] focus:outline-none transition-all ${errors.name ? 'border-red-500' : ''}`}
                      placeholder="Name"
                      {...register("name", { required: true })}
                    />
                    <label htmlFor="name" className="absolute left-3 top-0 -translate-y-1/2 bg-white px-1 text-[11px] font-medium text-slate-500 transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:bg-transparent peer-placeholder-shown:text-sm peer-focus:top-0 peer-focus:bg-white peer-focus:text-[11px] peer-focus:text-[#0f766e]">
                      Full Name
                    </label>
                  </div>
                  ...
                  <div className="relative group">
                    <input
                      ...
                      className={`peer block w-full px-4 py-3 bg-white border border-slate-300 rounded-lg text-sm text-slate-900 placeholder-transparent focus:border-[#0f766e] focus:ring-1 focus:ring-[#0f766e] focus:outline-none transition-all ${errors.reEnterPassword ? 'border-red-500' : ''}`}
                      placeholder="Confirm Password"
                      {...register("reEnterPassword", { required: true })}
                    />
                    <label htmlFor="reEnterPassword" className="absolute left-3 top-0 -translate-y-1/2 bg-white px-1 text-[11px] font-medium text-slate-500 transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:bg-transparent peer-placeholder-shown:text-sm peer-focus:top-0 peer-focus:bg-white peer-focus:text-[11px] peer-focus:text-[#0f766e]">
                      Confirm Password
                    </label>
                    <button type="button" onClick={() => setShowReEnterPassword(!showReEnterPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] font-bold text-[#0f766e] hover:text-[#115e59] px-2 py-1 bg-white/50 rounded">
      ...
      <div className="hidden lg:block w-[55%] h-full relative overflow-hidden bg-black">
        ...
        {/* === BRANDING (Right Side Text - Fly in from RIGHT) === */}
        <div className={`
            absolute bottom-16 right-16 max-w-lg text-right
            text-white
            transition-all duration-1000 ease-out
            ${mounted ? 'translate-x-0 opacity-100' : 'translate-x-40 opacity-0'}
        `}>
          <h1 className="text-5xl font-bold mb-4 tracking-tight drop-shadow-lg">
            Join the Future<span className="text-teal-400">.</span>
          </h1>
          <p className="text-white/90 text-xl leading-relaxed font-light drop-shadow-md">
Create your account to manage procedures, events, appointments, profiles, dashboards, and secure patient communication easily.          </p>
          {/* Decorative Dots */}
